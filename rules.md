...
- **Rule 14: Virtual Environment Package Installation**: All Python packages must be installed exclusively inside the project's virtual environment using `venv\Scripts\python.exe -m pip install <package>` from the `backend/` directory. Never install packages to the global Python environment. Redundant or unused packages should only be removed after explicit confirmation that they are not referenced anywhere in the codebase — removing packages carelessly can silently break transitive dependencies. When in doubt, leave them and annotate the concern in `requirements.txt` comments instead.
